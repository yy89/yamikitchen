...
less.Parser = function Parser(env) {
    ...
    function getLocation(index, inputStream) {
        ...
        if (typeof index === 'number') {
            line = (inputStream.slice(0, index).matc../g) || "").length;
        }
    ...
    return parser = {
        ...
        parsers: {
            ...
            combinator: function () {
                ...
                if (c === '>' || c === '+' || c === '~' || c === '|') {
                    i++;
                    while (input.charAt(i).matc../)) { i++; }
                    return new(tree.Combinator)(c);
                } else if (input.charAt(i - 1).matc../)) {
...
tree.functions = {
    ...
    escape: function (str) {
        return new(tree.Anonymous)(encodeURI(str.value).replace(/=/g, "%3D").replace(/:/g, "%3A").replace(/#/g, "%23").replace(/;/g, "%3B").replac../g, "%28").replac../g, "%29"));
    },
...
tree.Color.prototype = {
    ...
    //Adapted from http://mjijackson.com/20../rgb-to-hsl-and-rgb-to-hsv-color-model-conversion-algorithms-in-javascript
    toHSV: function () {
        var r = this.rgb[0] / 255,
            g = this.rgb[1] / 255,
            b = this.rgb[2] / 255,
...
tree.Comment.prototype = {
    ...
    isSilent: function(env) {
        var isReference = (this.currentFileInfo && this.currentFileInfo.reference && !this.isReferenced),
            isCompressed = env.compress && !this.value.matc../\*!/);
...
// http://www.w3.o../css3-values/#absolute-lengths
            ...
            if (directiveNode.name === "@charset") {
                ...
                if (this.charset) {
                    if (directiveNode.debugInfo) {
                        var comment = new tree.Comment("/* " + directiveNode.toCSS(this._env).replac../g, "")+" */\n");
...
(function (tree) {
    ...
    tree.sourceMapOutput.prototype.normalizeFilename = function(filename) {
        ...
        return (this._sourceMapRootpath || "") + filename.replac../g, '/');
    };
...
function extractId(href) {
    return href.replace(/^[a-z-]+:\/+?[../, '' )  // Remove protocol & domain
        ...
        .replac../g,                 ':'); // Replace dots with colons(for valid id)
}
...
function extractUrlParts(url, baseUrl) {
    ...
    if (urlParts[3]) {
        directories = urlParts[3].replac../g, "/").split("/");
